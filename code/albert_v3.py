...
class AgentState(TypedDict):
    ...
    sql_result: str
    omdb_result: str
    web_result: str  # ← FIXED: Added missing field
    
    # Conversation context
    original_question: str  # ← NEW: Store original question
    clarification_history: list  # ← NEW: Track clarifications
    user_intent: str  # ← NEW: Detected intent
    
    # Control flags
    needs_clarification: bool
    clarification_question: str
    needs_sql: bool  # ← NEW: Explicit SQL flag
    needs_omdb: bool  # ← NEW: Explicit OMDB flag
    needs_web: bool
    
    # Metadata
    sources_used: list
    confidence_score: float  # ← NEW: Quality metric
    current_step: str
...
def intent_analyzer_node(state: AgentState) -> dict:
    """NEW: Analyze user intent to decide which tools to use"""
    original_q = state.get("original_question", "")
    clarif_history = state.get("clarification_history", [])
...
def sql_agent_node(state: AgentState) -> dict:
    """IMPROVED: Execute SQL with better prompting"""
    catalog_info = format_catalog_for_llm(state["db_catalog"])
    original_q = state.get("original_question", "")
    clarif_history = state.get("clarification_history", [])
...
def web_search_node(state: AgentState) -> dict:
    """IMPROVED: Search web only if explicitly needed"""
...
def synthesize_node(state: AgentState) -> dict:
    """IMPROVED: Synthesize with confidence scoring"""
    ...
    web_result = state.get("web_result", "{}")  # ← FIXED: Now exists in state
...
def quality_check_node(state: AgentState) -> dict:
    """NEW: Check if answer quality is sufficient"""
...
@st.cache_resource
def build_agent():
    """Build and compile the enhanced workflow"""
    workflow = StateGraph(AgentState)
    
    # Add all nodes
    workflow.add_node("get_schema", get_schema_node)
    workflow.add_node("clarify_question", clarify_question_node)
    workflow.add_node("intent_analyzer", intent_analyzer_node)  # ← NEW
    ...
    workflow.add_node("quality_check", quality_check_node)  # ← NEW
    ...
    workflow.add_edge("wait_user", END)
